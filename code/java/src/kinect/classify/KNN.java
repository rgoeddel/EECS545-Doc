...
public class KNN {
    ...
    // thresholds values
    List<Double> thresholdLevels;
    List<String> thresholdLabels;
    List<Integer> thresholdUpdateSteps;
    
    int Km;
    int dim;

    // constants
    final double thresholdLambda = 0.8;
    final double thresholdConstant = 0.3;
	...
	this.dim = dim;
	thresholdLevels = new ArrayList<Double>();
	thresholdLabels = new ArrayList<String>();
	...
	ArrayList<ConfidenceLabel> shapeThresholds = 
	    new ArrayList<ConfidenceLabel>();
	
	for (int i = 0; i < thresholdLabels.size(); i++) {
	    String s = thresholdLabels.get(i);
	    double conf = thresholdLevels.get(i);
    ...
    public void updateThreshold(String label, int direction) {
	int index;
	...
	if ((index = thresholdLabels.indexOf(label)) >= 0) {
	    double threshold = thresholdLevels.get(index);
	    int updatestep = thresholdUpdateSteps.get(index) + 1;
	    System.out.print("Changing " + label + " threshold from " + threshold + " to ");
	    thresholdUpdateSteps.set(index, updatestep);
	    
	    threshold = threshold + Math.pow(thresholdLambda, updatestep)*thresholdConstant*((double)direction);
	    
	    if (threshold < minThresholdDefault)
		threshold = minThresholdDefault;
	    else if (threshold > maxThresholdDefault)
		threshold = maxThresholdDefault;
	    
	    
	    thresholdLevels.set(index, threshold);
	    System.out.println(threshold);
	} else {
	    System.out.println("ERROR: label:" + label + 
			       " not found for threshold update");
	}
    }
